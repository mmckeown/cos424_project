...
search_params = {
  'author' : '',
  'title' : '"Princeton University"', # multi-word titles should always be quoted (it seems?)
}
...
for i,elem in enumerate(root[2]): # iterate over individual query responses (documents)  
	...
	if(pdfpage_elems):
		...
		try:
		  file = open(os.path.join(output_dir, search_params['title'], 'response%d.pdf' % i), 'w')
		except IOError:
		  os.makedirs(os.path.join(output_dir, search_params['title']))
		  file = open(os.path.join(output_dir, search_params['title'], 'response%d.pdf' % i), 'w')
